...
public class RequestDealThread extends Thread {
    ...
    private static final String TAG = RequestDealThread.class.getSimpleName();
    ...
    private void sendLocalHeaderAndCache(int rangeStart, int rangeEnd, int fileLength, byte[] audioCache)
        ...
        String httpString = OkHttpUtil.genResponseHeader(rangeStart, rangeEnd, fileLength);
    ...
    private void processRequest(HttpURLConnection request, Socket client) throws IllegalStateException, IOException {
        ...
        try {
            ...
            Log.i(TAG, "原始请求Range起始值：" + originRangeStart + " 本地缓存长度：" + fileUtils.getLength());
            // 缓存的文件大小，数据库中无返回-1
            ...
            if (fileUtils.isEnable() && originRangeStart < fileUtils.getLength()) {
                audioCache = fileUtils.read(originRangeStart, Constants.AUDIO_BUFFER_MAX_LENGTH);
                Log.i(TAG, "本地已缓存长度（跳过）:" + audioCache.length);
            ...
            // 缓存不够。或者数据库没有文件大小
                ...
                if (cacheFileSize <= 0) {
                    Log.d(TAG, "数据库未包含文件大小，发送请求");
                    realResponse = OkHttpUtil.send(request);
                ...
                if (!isCacheEnough) {
                    ...
                    while (Thread.currentThread() == MediaPlayerProxy.downloadThread
                        ...
                        if (System.currentTimeMillis() / 1000 % 2 == 0) {
                            if (isPrint) {
                                Log.d(TAG, "Cache Size:" + readBytes + " File Start:" + fileBufferLocation
    ...
    private int getContentLength(HttpURLConnection response) {
        ...
        if (range != null) {
            ...
            Log.d(TAG, "getContentLength1: "+contentLength);
        } else {
            contentLength = request.getContentLength();
            Log.d(TAG, "getContentLength2: "+contentLength);
        }
        if (contentLength != 0) {
